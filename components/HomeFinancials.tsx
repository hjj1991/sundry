...
export default function HomeFinancials({financialProductType}: { financialProductType: string }) {
    ...
    return (
        <div className="p-4">
            <div
                className="text-2xl font-bold mb-4">{financialProductType === 'INSTALLMENT_SAVINGS' ? 'Top 10 적금' : 'Top 10 예금'}</div>
            <Table className="w-full border border-gray-300 dark:border-gray-600 rounded-lg overflow-hidden shadow-md">
                <TableHeader>
                    <TableRow>
                        <TableHead
                            className="bg-teal-100 dark:bg-teal-700 text-teal-700 dark:text-teal-300 border-b border-gray-300 dark:border-gray-600">회사명
                            / 상품명</TableHead>
                        <TableHead
                            className="bg-teal-100 dark:bg-teal-700 text-teal-700 dark:text-teal-300 border-b border-gray-300 dark:border-gray-600">기본
                            이율</TableHead>
                        <TableHead
                            className="bg-teal-100 dark:bg-teal-700 text-teal-700 dark:text-teal-300 border-b border-gray-300 dark:border-gray-600">최고
                            우대 이율</TableHead>
                    </TableRow>
                </TableHeader>
                <TableBody>
                    {data?.content.map((item, index) => (
                        <TableRow key={index}>
                            <TableCell className="py-2 px-4 text-gray-800 dark:text-gray-300">
                                <div className="flex flex-col">
                                    <span
                                        className="font-semibold text-indigo-600 dark:text-indigo-400">{item.financialCompany.companyName}</span>
                                    <span
                                        className="text-gray-600 dark:text-gray-400">{item.financialProductName}</span>
                                </div>
                            </TableCell>
                            <TableCell className="py-2 px-4 text-gray-800 dark:text-gray-300">
